...
for(k in 1:ksce){
  ...
  out.tmp.cont <- mclapply(1:repet, simu.one.PAR.mpi, k=k,rsd21=rsd21,rsdT1=rsdT1,mu.R1=mu.R1,mu.R2=mu.R2,mu.T=mu.T,sd.all=sd.all,n=n,mc.cores=no_cores)
  
  
  restmp <- matrix(unlist(out.tmp.cont),nrow=repet)
