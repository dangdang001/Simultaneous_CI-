# Purpose: Generating Pairwise method and Simu CI method result, mu_R1=95, mu_R2=105, mu_T=100
# Name: Donglei Yin
# Date: 2018.07

# Note: Parallel computing method was applied using R package "parallel"
#       and the (number of total cores available-1) cores were used in the calculation
...
sd.margin<-max((max(mu.R1,mu.R2,mu.T)-min(mu.R1,mu.R2,mu.T))/1.5,1)

sd.all <- unique(c(sd.margin,seq(2,14,2)))

sd.all<-sd.all[order(sd.all)]
# sd.all <- c(seq(8,10,2))
...
for(k in 1:ksce){
  ...
  simu.one.PAR.mpi <- function(i=1,k,rsd21,rsdT1,mu.R1,mu.R2,mu.T,sd.all,n,sd.margin){
    ...
    if(eFP1.res$value>p.level){
      ...
      rcd.simuCI1.tmp2 <- uniroot(function(delta) eFP1.int1(delta)-p.level, lower = 0, upper = 1.5*sd.ref*sd.margin)$root
    }
    ...
    rcd.simuCI1.tmp3 <- uniroot(function(delta) eFP1.int2(delta)-p.level, lower = 0, upper = 20*sd.ref*sd.margin)$root
    ...
    if(eFP1.res.mod$value>p.level){
      ...
      rcd.simuCI1.tmp5 <- uniroot(function(delta) eFP1.int1.mod(delta)-p.level, lower = 0, upper = 1*sd.margin)$root
    }
    ...
    rcd.simuCI1.tmp6 <- uniroot(function(delta) eFP1.int2.mod(delta)-p.level, lower = 0, upper = 20*sd.margin)$root
    ...
    if(eFP1.res.lf$value>p.level){
      ...
      rcd.simuCI1.tmp8 <- uniroot(function(delta) eFP1.int1.lf(delta)-p.level, lower = 0, upper = 1.5*sd.ref.lf*sd.margin)$root
    }
    ...
    rcd.simuCI1.tmp9 <- uniroot(function(delta) eFP1.int2.lf(delta)-p.level, lower = 0, upper = 20*sd.ref.lf*sd.margin)$root
  ...
  system.time(out.tmp.cont <- parSapply(cl, 1:repet, simu.one.PAR.mpi, k=k,rsd21=rsd21,rsdT1=rsdT1,mu.R1=mu.R1,mu.R2=mu.R2,mu.T=mu.T,sd.all=sd.all,n=n,sd.margin=sd.margin))
...
write.csv(Arcd, paste("Arcd_n",n,"_",mu.R1,"_",mu.R2,"_",mu.T,"_equal_var","_sdR1_",round(sd.all[1],2),"_",round(sd.all[ksce],2),"_rp",repet,"_",rundate,".csv",sep=""))
write.csv(summ, paste("summ_n",n,"_",mu.R1,"_",mu.R2,"_",mu.T,"_equal_var","_sdR1_",round(sd.all[1],2),"_",round(sd.all[ksce],2),"_rp",repet,"_",rundate,".csv",sep=""))
