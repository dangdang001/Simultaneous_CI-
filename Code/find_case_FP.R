# Purpose: Find the cases where pairwise comparison leads to false positive but simulatanous confidence interval approach succeed.
# Name: Donglei Yin
# Date: 2018.07
...
table(result$diff_FP)


index=which(result$diff_FP==TRUE, arr.ind=TRUE)
...
write.csv(example.data,file = "./example_FP_data.csv")
...
ggsave(plot,filename = "./Scatterplot_Fp.png", height=5, width=7, units='in', dpi=600)
...
write.csv(Pairwise.result,file = "./example_FP_pair.csv")
write.csv(Simu.CI,file = "./example_FP_simu.csv")
