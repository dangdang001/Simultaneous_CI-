...
for(k in 1:ksce){
  ...
  clusterExport(cl, c("simu.one.PAR.mpi","k","repet","rsd21","rsdT1","mu.R1","mu.R2","mu.T","sd.all","n"))
  
  clusterEvalQ(cl, {library(R2Cuba)})
  
  system.time(out.tmp.cont <- parLapply(cl, 1:repet, simu.one.PAR.mpi, k=k,rsd21=rsd21,rsdT1=rsdT1,mu.R1=mu.R1,mu.R2=mu.R2,mu.T=mu.T,sd.all=sd.all,n=n))
